...
public class Main {
    ...
    public static Car racerMark = new Car("QS739", "Subaru", 2, 22, 0.4, 0); // Нет нужны выносить поля в статику
    public static Car racerTom = new Car("RX502", "Toyota", 3, 25, 0.3, 0);
    public static Car racerJim = new Car("LT831", "BMW", 2, 30, 0.25, 0);

    public static Track myTrack = new Track(4,0.42); // Нет нужны выносить поля в статику
    public static int distance = 0; // Нет нужны выносить поля в статику
    ...
    public static void checkCar(Car car, Track track){
        // Нет нужны выносить поля в статику
    ...
    public static void circleRun(Car car, Track track){
        ...
        if (car.getCarDistance() % 4 > 0){ // зачем берем остаток от деление
    ...
    public static void startRace(){
        while (racerMark.getStops() < 3 || racerTom.getStops() < 3 || racerJim.getStops() < 3){ //  проверки будут идти  до тех пор пока все гонщики не завршили заезд
                checkCar(racerMark, myTrack); // и здесь 
                circleRun(racerMark, myTrack); // и здесь
                // выполняеются одинаковые проверки.
                checkCar(racerTom, myTrack);
                circleRun(racerTom, myTrack);

                checkCar(racerJim, myTrack);
                circleRun(racerJim, myTrack);
                // эти секции, чутка репототивны
                distance++;
        }
        result();
    }
